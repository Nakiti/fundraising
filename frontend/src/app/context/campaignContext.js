...
export const CampaignContextProvider = ({ organizationId, campaignId, children }) => {
   // Campaign data state
   ...
   useEffect(() => {
      const fetchCampaignData = async (campaignId, organizationId) => {
         ...
         try {
            ...
            const campaignDetailsResponse = await campaignService.getCampaignWithDetails(campaignId);
            ...
            setCampaignDetails({
               externalName: campaignDetailsResponse.data.externalName || "",
               internalName: campaignDetailsResponse.data.internalName || "",
               goal: campaignDetailsResponse.data.goal || 0,
               url: campaignDetailsResponse.data.url || "",
               defaultDesignation: campaignDetailsResponse.data.defaultDesignation || 0,
               showLeaderboard: campaignDetailsResponse.data.showLeaderboard || true,
               allowAnonymousDonations: campaignDetailsResponse.data.allowAnonymousDonations || true,
               anonymizeAllDonations: campaignDetailsResponse.data.anonymizeAllDonations || false
            });
   
            // Step 2.5: Set question inputs state from backend data
            setQuestionInputs({
               phone: campaignDetailsResponse.data.showPhone || false,
               title: campaignDetailsResponse.data.showTitle || false,
               suffix: campaignDetailsResponse.data.showSuffix || false,
               companyorganizationname: campaignDetailsResponse.data.showCompanyName || false,
               websiteurl: campaignDetailsResponse.data.showWebsiteUrl || false
            });
   ...
   const handleCampaignDetailsChange = (e) => {
      ...
      if (['externalName', 'internalName', 'goal', 'url'].includes(name)) {
      ...
      if (['showLeaderboard', 'allowAnonymousDonations', 'anonymizeAllDonations'].includes(name)) {
         setPageHasChanges('settings', true);
      }
   };
