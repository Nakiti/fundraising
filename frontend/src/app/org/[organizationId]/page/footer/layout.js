"use client"
import FooterPageDisplay from "./components/footerPageDisplay"
import { useContext, useState } from "react"
import { FooterPageContext } from "@/app/context/organizationPages/footerPageContext"
import { Services } from "@/app/services"
import { AuthContext } from "@/app/context/authContext"
import Navbar from "../components/navbar"
import { validateActiveSections } from "@/app/utils/pageValidation"

const FooterLayout = ({params, children}) => {
   const [error, setError] = useState(false)
   const [errorMessage, setErrorMessage] = useState("")
   const [isSaving, setIsSaving] = useState(false)
   const [isPublishing, setIsPublishing] = useState(false)
   const [isDeactivating, setIsDeactivating] = useState(false)
   const {currentUser} = useContext(AuthContext)
   const organizationId = params.organizationId
   const {inputs, sections, setInputs} = useContext(FooterPageContext)

   const links = [
      `/org/${organizationId}/page/footer`,
      `/org/${organizationId}/page/footer/design`
   ]

   const handleSave = async() => {
      // Reset error state
      setError(false)
      setErrorMessage("")
      setIsSaving(true)

      try {
         // Validate all active sections before saving
         const validation = validateActiveSections('footer', sections, inputs)
         
         if (!validation.isValid) {
            setError(true)
            setErrorMessage(`Please fill in the following required fields: ${validation.errors.join(", ")}`)
            setIsSaving(false)
            return
         }
         
         // Use the service to save footer page
         const result = await Services.Update.FooterPage.saveFooterPage(inputs.id, inputs, sections)
         
         // Success feedback
         console.log(result.message)
         setIsSaving(false)
         
         // Optional: Show success message or toast notification
         // You can add a toast notification here if you have a notification system

      } catch (err) {
         console.error("Error saving footer page:", err)
         setError(true)
         setErrorMessage(err.message || "Failed to save footer page. Please try again.")
         setIsSaving(false)
      }
   }

   const handlePublish = async() => {
      // Reset error state
      setError(false)
      setErrorMessage("")
      setIsPublishing(true)

      try {
         // Validate all active sections for publishing
         const validation = validateActiveSections('footer', sections, inputs)
         
         if (!validation.isValid) {
            setError(true)
            setErrorMessage(`Please fill in the following required fields to publish: ${validation.errors.join(", ")}`)
            setIsPublishing(false)
            return
         }
         
         // Use the service to publish footer page
         const result = await Services.Update.FooterPage.publishFooterPage(inputs.id, inputs, sections)
         
         // Update the context to reflect the new active status
         setInputs(prev => ({ ...prev, active: true }))

         // Success feedback
         console.log(result.message)
         setIsPublishing(false)
         
         // Optional: Show success message or toast notification
         // You can add a toast notification here if you have a notification system

      } catch (err) {
         console.error("Error publishing footer page:", err)
         setError(true)
         setErrorMessage(err.message || "Failed to publish footer page. Please try again.")
         setIsPublishing(false)
      }
   }

   const handleDeactivate = async() => {
      // Reset error state
      setError(false)
      setErrorMessage("")
      setIsDeactivating(true)

      try {
         // Use the service to deactivate footer page
         const result = await Services.Update.FooterPage.deactivateFooterPage(inputs.id, inputs, sections)
         
         // Update the context to reflect the new inactive status
         setInputs(prev => ({ ...prev, active: false }))

         // Success feedback
         console.log(result.message)
         setIsDeactivating(false)
         
         // Optional: Show success message or toast notification
         // You can add a toast notification here if you have a notification system

      } catch (err) {
         console.error("Error deactivating footer page:", err)
         setError(true)
         setErrorMessage(err.message || "Failed to deactivate footer page. Please try again.")
         setIsDeactivating(false)
      }
   }

   return (
      <div className="w-full h-full bg-gray-50">
         <Navbar 
            organizationId={organizationId} 
            links={links} 
            title={"Footer Design"} 
            handleSave={handleSave} 
            handlePublish={handlePublish} 
            handleDeactivate={handleDeactivate}
            isSaving={isSaving} 
            isPublishing={isPublishing}
            isDeactivating={isDeactivating}
            status={inputs.active ? 'active' : 'draft'}
            pageType="footer"
         />
         
         {/* Error Display */}
         {error && (
            <div className="mx-4 mt-4 p-4 bg-red-50 border border-red-200 rounded-lg">
               <div className="flex">
                  <div className="flex-shrink-0">
                     <svg className="h-5 w-5 text-red-400" viewBox="0 0 20 20" fill="currentColor">
                        <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clipRule="evenodd" />
                     </svg>
                  </div>
                  <div className="ml-3">
                     <h3 className="text-sm font-medium text-red-800">Error saving page</h3>
                     <div className="mt-2 text-sm text-red-700">
                        {errorMessage}
                     </div>
                  </div>
               </div>
            </div>
         )}
         
         <div className="p-4 space-y-4">
            <div className="flex flex-col xl:flex-row gap-4 h-[calc(100vh-120px)]">
               {/* Left Panel - Content Editor */}
               <div className="xl:w-1/4 lg:w-1/3">
                  <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-2 h-full overflow-y-auto">
                     <h2 className="text-lg font-semibold text-gray-900 mb-4 sticky top-0 bg-white py-2">Content Editor</h2>
                     <div className="space-y-4">
                        {children}
                     </div>
                  </div>
               </div>
               
               {/* Right Panel - Preview */}
               <div className="xl:w-3/4 lg:w-2/3">
                  <div className="bg-white rounded-xl shadow-sm border border-gray-100 h-full">
                     <div className="h-full overflow-y-auto">
                        <div className="border border-gray-200 rounded-lg overflow-hidden bg-white shadow-inner">
                           <FooterPageDisplay />
                        </div>
                     </div>
                  </div>
               </div>
            </div>
         </div>
      </div>
   )
}

export default FooterLayout
