...
const Navbar = ({active, handleActiveChange, handlePublish, handleSave, handleDeactivate, title, links, mode}) => {
   ...
   return (
      <div className="border-b border-gray-100 bg-blue-900 shadow-sm bg-white text-black">
         ...
         <div className="flex items-center justify-between w-11/12 mx-auto py-4">
            ...
            <div className="flex space-x-4 text-md">
               {status == "inactive" ? 
                  <button 
                     className="border-2 border-gray-100 hover:bg-gray-600 py-1 px-8 rounded-md text-gray-100"
                     onClick={handleSave}
                  >
                     Save
                  </button> :
         ...
         <div className="flex flex-row space-x-8 w-11/12 mx-auto mt-8 text-gray-100 mb-1">
            {mode == "new" ? <Link
               className={`cursor-pointer text-md font-semibold border-b-2 px-4 ${pathName.split("/")[4] == links[0].split("/")[4] ? "border-white" : "border-transparent"}`}
               href={links[0]}
            >
               Details
            </Link> 
            :
            <Link
               className={`cursor-pointer text-md font-semibold border-b-2 px-4 ${pathName.split("/")[5] == links[0].split("/")[5] ? "border-white" : "border-transparent"}`}
               href={links[0]}
            >
               Details
            </Link> 
